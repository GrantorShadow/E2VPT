...
class Trainer():
    ...
    def append_adversarial_prompt(self, adv_model1, robust_model2): # adv, model
        """ Appends adversarial prompts and returns prompt concat adv prompt """
        # Extract weights from both models
        adv_prompts = {k: v.clone() for k, v in adv_model1.named_parameters()}
        rand_prompts = {k: v.clone() for k, v in robust_model2.named_parameters()}
    ...
    def train_classifier(self, train_loader, val_loader, test_loader, adv_model):
        """
        ...
        for epoch in range(total_epoch):
            if self.mode == "def":
                # append the adversarial prompts layerwise
                self.model = self.append_adversarial_prompt(adv_model,
                                                            self.model)
                logger.info("Adversarial Prompts successfully appended")
            ...
            for idx, input_data in enumerate(train_loader):
                ...
                train_loss, _ = self.forward_one_batch(X, targets, True) # why are we using one batch at a time?
                ...
                if (idx + 1) % log_interval == 0:
                    ...
                    wandb.log({
                        'epoch_avg_train_loss': train_loss.avg,
                        'epoch': epoch + 1
                    })
    ...
    def eval_classifier(self, data_loader, prefix, save=False):
        ...
        for idx, input_data in enumerate(data_loader):
            ...
            if (idx + 1) % log_interval == 0:
                ...
                wandb.log({
                f"{prefix}_batch_loss": losses.val,
                f"{prefix}_data_time": data_time.val,
                f"{prefix}_batch_time": batch_time.val,
                f"{prefix}_index": idx + 1,
                })
        ...
        # Log final evaluation summary to wandb
        wandb.log({
        f"{prefix}_avg_loss": losses.avg,
        f"{prefix}_avg_data_time": data_time.avg,
        f"{prefix}_avg_batch_time": batch_time.avg,
        })
